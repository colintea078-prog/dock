...
import { occursOn } from './dates.js?v=84';
import { todoForDay, todoIsDone, announceTodo } from './checklist.js?v=84';
...
export function mountCalendar(root, { cfg, store }) {
  ...
  async function draw() {
    ...
    for (let i = 0; i < 42; i++) {
      ...
      if (hits.length) {
        ...
        flag = st && st.img
          ? `<img class="cal-ev" src="./assets/pack/${st.img}.webp?v=84" alt="">`
  ...
  function evLine(k) {
    ...
    return `<div class="cal-evline">${hits.map(h => {
      ...
      const pic = st && st.img
        ? `<img src="./assets/pack/${st.img}.webp?v=84" alt="">` : '';
  ...
  function spendBlock(k) {
    ...
    return `
      <div class="sh-sec">
        ...
        ${rows.map(r => {
          const c = catOf(r.cat);
          const icon = c ? `<img src="./assets/cat/${c.icon}.webp?v=84" alt="">` : '';
          return `
            <div class="sh-led">
              ${icon}
