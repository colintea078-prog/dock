...
import { occursOn } from './dates.js?v=74';
...
export function mountCalendar(root, { cfg, store }) {
  ...
  async function draw() {
    ...
    for (let i = 0; i < 42; i++) {
      ...
      if (hits.length) {
        ...
        flag = st && st.img
          ? `<img class="cal-ev" src="./assets/pack/${st.img}.webp?v=74" alt="">`
      ...
      const moodPip = rec.mood
        ? `<img class="cal-mood" src="./assets/mood/${rec.mood}.webp?v=74" alt="">`
        : '';
      cell.innerHTML = `
        ${moodPip}
        ${back}
  ...
  function evLine(k) {
    ...
    return `<div class="cal-evline">${hits.map(h => {
      ...
      const pic = st && st.img
        ? `<img src="./assets/pack/${st.img}.webp?v=74" alt="">` : '';
        ...
        <div class="sheet-moods">
          ${MOODS.map(mo => `
            <button class="mood-pick ${picked === mo.id ? 'on' : ''}" data-mood="${mo.id}">
              <img src="./assets/mood/${mo.id}.webp?v=74" alt="">
              <span>${mo.name}</span>
            </button>`).join('')}
        </div>
