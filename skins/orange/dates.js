import { makeFab } from './fab.js?v=81';
/* 日期备忘录。
 *
...
export function mountDates(root, { cfg, store }) {
  ...
  const sticker = (t, cls) => img(t)
    ? `<img class="${cls} pic" src="./assets/pack/${img(t)}.webp?v=81" alt="${escapeHtml(t)}">`
  ...
  function draw() {
    ...
    if (used.length > 1) {
      ...
      used.forEach(t => {
        ...
        if (img(t)) {
          b.classList.add('pic');
          b.innerHTML = `<img src="./assets/pack/${img(t)}.webp?v=81" alt="${escapeHtml(t)}">`;
        } else {
            ...
            ${TAGS.map(t => {
              ...
              return img(t)
                ? `<button class="dt-opt pic ${on}" data-tag="${t}">
                     <img src="./assets/pack/${img(t)}.webp?v=81" alt="${escapeHtml(t)}"></button>`
